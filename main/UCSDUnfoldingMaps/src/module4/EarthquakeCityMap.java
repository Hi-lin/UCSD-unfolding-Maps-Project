...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		int x=150;
		fill(0, 255, 0);
		rect(x-5, 125-5, 10, 10);
		fill(255, 0, 0);
		triangle(x, 175-10, x+7, 175+5, x-7, 175+5);
		fill(0, 0, 255);
		ellipse(x, 225, 10, 10);
		
		fill(0, 0, 0);
		text("City", 50, 125);
		text("LandQuake", 50, 175);
		text("OceanQuake", 50, 225);
	}
		...
		for(Marker m: quakeMarkers) {
			if(m instanceof OceanQuakeMarker) {
				ocean++;
					...
					if(!set.containsKey(m.getProperty("country"))) {
						set.put((String) m.getProperty("country"), 1);
					}
					else {
						set.replace((String) m.getProperty("country"), set.get((String) m.getProperty("country"))+1);
					}
			}
		}
		System.out.println("hi");
		System.out.print("Ocean : "+ocean);
